...
type EventStore interface {
	...
	GetUnconsolidatedBySession(ctx context.Context, sessionID string) ([]Event, error)
...
// GetUnconsolidatedBySession retrieves unconsolidated events for a session.
// Used by consolidation paths to avoid re-processing already-consolidated events.
func (s *SQLiteEventStore) GetUnconsolidatedBySession(ctx context.Context, sessionID string) ([]Event, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, session_id, timestamp, source, actor, kind, content, metadata, project_id, provenance, created_at
		FROM events
	...
	`, sessionID)
	if err != nil {
		return nil, fmt.Errorf("querying unconsolidated events by session: %w", err)
	}
	defer rows.Close()
...
// parseTime parses a time string trying multiple formats.
// Handles both RFC3339Nano (our write path) and SQLite's CURRENT_TIMESTAMP format.
