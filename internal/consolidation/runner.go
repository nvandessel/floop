...
func (r *Runner) Run(ctx context.Context, evts []events.Event, s store.GraphStore, opts RunOptions) (*RunResult, error) {
	...
	// Collect ALL input event IDs for callers to mark as consolidated.
	// This includes events that didn't match any heuristic pattern —
	// they've been scanned and should not be re-processed.
	result.SourceEventIDs = make([]string, 0, len(evts))
	for _, evt := range evts {
		result.SourceEventIDs = append(result.SourceEventIDs, evt.ID)
	}
